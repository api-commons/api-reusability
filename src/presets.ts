...
const PROVIDERS: PresetSet[] = [
  ...
  file('paypal', 'PayPal — payments'),
  file('visa', 'Visa — payments'),
  file('zoom', 'Zoom — communications'),
  file('linkedin', 'LinkedIn — social'),
  file('figma', 'Figma — design'),
  file('gitlab', 'GitLab — developer tools'),
  file('discord', 'Discord — communications'),
  file('youtube', 'YouTube — media'),
  file('google', 'Google — cloud'),
  file('salesforce', 'Salesforce — CRM'),
  file('docusign', 'DocuSign — documents'),
  file('datadog', 'Datadog — observability'),
  file('auth0', 'Auth0 — identity'),
  file('bunq', 'bunq — banking'),
  file('amadeus', 'Amadeus — travel'),
  file('factset', 'FactSet — financial data'),
  file('microsoft-graph', 'Microsoft Graph — productivity'),
  file('palo-alto', 'Palo Alto Networks — security'),
  file('webex', 'Cisco Webex — communications'),
  file('avalara', 'Avalara — tax'),
  file('vapi', 'Vapi — AI'),
